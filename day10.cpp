...
class seq : public sCompare {
	public:
		void Execution() {if (type == 1) ans = (RAM[s2] == RAM[s3]); else ans = (RAM[s2] == s3);}
};

class sge : public sCompare {
	public:
		void Execution() {if (type == 1) ans = (RAM[s2] >= RAM[s3]); else ans = (RAM[s2] >= s3);}
};

class sgt : public sCompare {
	public:
		void Execution() {if (type == 1) ans = (RAM[s2] > RAM[s3]); else ans = (RAM[s2] > s3);}
};

class sle : public sCompare {
	public:
		void Execution() {if (type == 1) ans = (RAM[s2] <= RAM[s3]); else ans = (RAM[s2] <= s3);}
};

class slt : public sCompare {
	public:
		void Execution() {if (type == 1) ans = (RAM[s2] < RAM[s3]); else ans = (RAM[s2] < s3);}
};

class sne : public sCompare {
	public:
		void Execution() {if (type == 1) ans = (RAM[s2] != RAM[s3]); else ans = (RAM[s2] != s3);}
};
...
class beq : public branch {
	public:
		bool Execution() {if (type == 1) b = (RAM[s1] == RAM[s2]); else b = (RAM[s1] == s2);return b;}
};
...
class beqz : public branchz {
	public:
		bool Execution() {b = (RAM[s1] == 0); return b;}
};

class bnez : public branchz {
	public:
		bool Execution() {b = (RAM[s1] != 0); return b;}
};

class blez : public branchz {
	public:
		bool Execution() {b = (RAM[s1] <= 0); return b;}
};

class bgez : public branchz {
	public:
		bool Execution() {b = (RAM[s1] >= 0); return b;}
};

class bgtz : public branchz {
	public:
		bool Execution() {b = (RAM[s1] > 0); return b;}
};

class bltz : public branchz {
	public:
		bool Execution() {b = (RAM[s1] < 0); return b;}
};
...
class jr {
	...
	public:
		...
		void Execution() {}
		int Write_Back() {return RAM[s1] - 1;}
};
...
class jal {
	...
	public:
		...
		void Execution() {}
		int Write_Back() {RAM[31] = l;return label[t];}
};
...
int main(int agrc, char * agrv[]) {
	...
	while (true) {
		...
		if (whetherjump == 1) {
			for (int k = 0; k <= 4; ++k) { 
				status[k] = 0; s[k] = ""; name[k] = 0;
				branch[k] = 0; origin[k] = 0; 
				whether[k] = 0; bpc[k] = 0; 
			}
			whetherjump = 0;
		}
		if (hazard == 1 || hazard == 2) {
			bool b = true;
			for (int k = 0; k <= 4; ++k) if (status[k] == 1) b = false;
			if (b == true) {
				i = 4;
				for (int k = 0; k <= 4; ++k) {
					status[k] = 0; s[k] = ""; name[k] = 0;
				}
				for (int k = 0; k <= 33; ++k) loc[k] = 0;
				if (hazard == 1) {
					if (laststone == 1) j++;
					laststone = 0;
				}
				else --j;
				//while (label[str[j]] != 0) ++j;
				hazard = 0;
			}
		}else {
			++j;
		...
		for (int k = 4; k >= 0; --k) {
			...
			if (k == 0) { 
				...
				switch(name[t]) {
					...
					case 23: bpc[t] = 1;beqdata[t].Data_Preparation(s[t]);break;
					case 24: bpc[t] = 1;bnedata[t].Data_Preparation(s[t]);break;
					case 25: bpc[t] = 1;bgedata[t].Data_Preparation(s[t]);break;
					case 26: bpc[t] = 1;bledata[t].Data_Preparation(s[t]);break;
					case 27: bpc[t] = 1;bgtdata[t].Data_Preparation(s[t]);break;
					case 28: bpc[t] = 1;bltdata[t].Data_Preparation(s[t]);break;
					case 29: bpc[t] = 1;beqzdata[t].Data_Preparation(s[t]);break;
					case 30: bpc[t] = 1;bnezdata[t].Data_Preparation(s[t]);break;
					case 31: bpc[t] = 1;bgezdata[t].Data_Preparation(s[t]);break;
					case 32: bpc[t] = 1;blezdata[t].Data_Preparation(s[t]);break;
					case 33: bpc[t] = 1;bgtzdata[t].Data_Preparation(s[t]);break;
					case 34: bpc[t] = 1;bltzdata[t].Data_Preparation(s[t]);break;
				...
				switch(name[t]) {
					...
					case 23: f = beqdata[t].Execution();break;
					case 24: f = bnedata[t].Execution();break;
					case 25: f = bgedata[t].Execution();break;
					case 26: f = bledata[t].Execution();break;
					case 27: f = bgtdata[t].Execution();break;
					case 28: f = bltdata[t].Execution();break;
					case 29: f = beqzdata[t].Execution();break;
					case 30: f = bnezdata[t].Execution();break;
					case 31: f = bgezdata[t].Execution();break;
					case 32: f = blezdata[t].Execution();break;
					case 33: f = bgtzdata[t].Execution();break;
					case 34: f = bltzdata[t].Execution();break;
				...
				if (bpc[t] == 1) bpc[t] = 0;
				else if (tmp != 0) {laststone = 1; j = tmp;}
				if (name[t] == 46 && mark != -1) {
					cout <<sum1<<' '<<sum2<<' '<<(double)sum1 / (double)sum2<<endl;
					return mark;
				}
			}
		}
